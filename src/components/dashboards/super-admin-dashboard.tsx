...
export function SuperAdminDashboard() {
  ...
  const fetchAnalytics = useCallback(async () => {
    ...
    useEffect(() => {
      ...
      fetchPlans();
      fetchAnalytics();
    }, [fetchSchools, fetchCodes, fetchLogs, fetchNotifications, fetchPlans, fetchAnalytics]);
  ...
  const maxStudentCount = Math.max(...schools.map(s => s._count?.students || 0), 1);
   ...
   const planPriceMap = plans.reduce((acc, plan) => ({
    ...
    const platformRevenue = analytics?.financialData?.totalRevenue || Object.values(revenueByPlan).reduce((a, b) => a + b, 0);
    ...
    // Use real revenue trend from analytics if available, otherwise fallback to code-based recentMonths
    const analyticsTrend = analytics?.revenueTrend?.map((item: any) => {
    ...
    const recentMonths = analyticsTrend.length > 0 ? analyticsTrend.slice(-monthsToShow) : monthRevenueArray.slice(-monthsToShow);
    const maxRevenue = Math.max(...recentMonths.map((r: any) => r.value), 1);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-3 sm:gap-4 lg:grid-cols-3 xl:grid-cols-6">
        ...
        <KpiCard title="Active Users" value={(analytics?.schoolOverview?.totalStudents + analytics?.schoolOverview?.totalTeachers) || (totalStudents + totalTeachers)} icon={GraduationCap} iconBgColor="bg-blue-100" iconColor="text-blue-600" changeLabel="students" />
        <KpiCard title="Teachers" value={analytics?.schoolOverview?.totalTeachers || totalTeachers} icon={Users} iconBgColor="bg-purple-100" iconColor="text-purple-600" changeLabel="registered" />
        <KpiCard title="Platform Revenue" value={`₦${(platformRevenue / 1000000).toFixed(1)}M`} icon={TrendingUp} iconBgColor="bg-amber-100" iconColor="text-amber-600" changeLabel="total revenue" />
