...
import {
  ...
  Megaphone, IdCard, TrendingUp, Clock, BookOpen, Shield,
  Award, AlertTriangle, CheckCircle2, UserCheck, Plus, ChevronRight,
  BarChart3, ArrowUpRight, ArrowDownRight, CircleDot, RefreshCw, XCircle, Zap
} from 'lucide-react';
      ...
      if (examsRes.status === 'fulfilled' && examsRes.value.ok) {
       ...
       try {
         ...
         if (analyticsRes.ok) {
           ...
           if (analyticsJson.data?.schoolOverview) {
             ...
             const health = overview.operationalHealth || { attendanceHealth: 0, financialHealth: 0, academicHealth: 0 };
             ...
             setStats({
               ...
               previousRevenue: 0,
               attendanceHealth: health.attendanceHealth,
       ...
       async function fetchAuditLogs() {
        ...
        try {
          ...
          const res = await fetch(`/api/audit-logs?schoolId=${selectedSchoolId}&limit=8`);
          if (res.ok) {
          ...
          <CardHeader className="pb-2 border-b bg-white/40">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <CardTitle className="text-lg font-bold">Operational <span className="text-blue-600">Health</span></CardTitle>
                  <CardDescription className="text-xs font-semibold uppercase tracking-wider">Live School Performance Pulse</CardDescription>
                </div>
              </div>
              <div className="flex items-center gap-2">
                <Badge variant="outline" className="bg-white/90 text-[10px] font-bold py-1 px-3 rounded-lg border-blue-100 text-blue-700 shadow-sm">
                  <RefreshCw className="size-3 mr-1.5 animate-spin-slow" /> System Sync: Active
                </Badge>
              </div>
            </div>
          </CardHeader>
          <CardContent className="p-6">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              ...
              <div className="space-y-4">
                <div className="flex justify-between items-end">
                  <div className="space-y-1">
                    <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">Attendance Health</p>
                    <p className="text-2xl font-black text-emerald-600">{stats.attendanceHealth}%</p>
                  </div>
                  <Users className="size-4 text-emerald-500 mb-1 opacity-50" />
                </div>
                <div className="h-2 bg-emerald-100 rounded-full overflow-hidden">
                  <motion.div 
                    initial={{ width: 0 }}
                    animate={{ width: `${stats.attendanceHealth}%` }}
                    className="h-full bg-emerald-500"
                  />
                </div>
                <p className="text-[9px] font-bold text-muted-foreground leading-tight italic">Students accounted for today</p>
              </div>
              ...
              <div className="space-y-4">
                <div className="flex justify-between items-end">
                  <div className="space-y-1">
                    <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">Financial Health</p>
                    <p className="text-2xl font-black text-blue-600">{stats.financialHealth}%</p>
                  </div>
                  <Wallet className="size-4 text-blue-500 mb-1 opacity-50" />
                </div>
                <div className="h-2 bg-blue-100 rounded-full overflow-hidden">
                  <motion.div 
                    initial={{ width: 0 }}
                    animate={{ width: `${stats.financialHealth}%` }}
                    className="h-full bg-blue-500"
                  />
                </div>
                <p className="text-[9px] font-bold text-muted-foreground leading-tight italic">Verified vs Total recorded revenue</p>
              </div>
              ...
              <div className="space-y-4">
                <div className="flex justify-between items-end">
                  <div className="space-y-1">
                    <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">Academic Health</p>
                    <p className="text-2xl font-black text-purple-600">{stats.academicHealth}%</p>
                  </div>
                  <GraduationCap className="size-4 text-purple-500 mb-1 opacity-50" />
                </div>
                <div className="h-2 bg-purple-100 rounded-full overflow-hidden">
                  <motion.div 
                    initial={{ width: 0 }}
                    animate={{ width: `${stats.academicHealth}%` }}
                    className="h-full bg-purple-500"
                  />
                </div>
                <p className="text-[9px] font-bold text-muted-foreground leading-tight italic">Published exams per class avg</p>
              </div>
            </div>
          </CardContent>
        </Card>
                  ...
                  <Card className="glass-panel border-0 overflow-hidden shadow-xl">
                    <CardHeader className="pb-3 border-b bg-white/40 flex flex-row items-center justify-between">
                      <div className="flex flex-col">
                        <span className="uppercase tracking-widest text-[10px] font-bold text-gray-400">Security & Operations</span>
                        <span className="text-sm font-black text-blue-600">Activity Log</span>
                      </div>
                      <Shield className="size-4 text-blue-500 animate-pulse" />
                    </CardHeader>
                    <CardContent className="p-0">
                      <ScrollArea className="h-[360px]">
                        <div className="divide-y divide-gray-100">
                          {auditLoading ? (
                            ...
                            auditLogs.map(log => (
                              <div key={log.id} className="p-4 hover:bg-blue-50/30 transition-colors group">
                                <div className="flex items-start gap-3">
                                  <div className={cn(
                                    ...
                                    log.action.includes('DELETE') ? "bg-red-100 text-red-600" : "bg-gray-100 text-gray-600"
                                  )}>
                                    <Clock className="size-4" />
                                  </div>
                                  <div className="flex-1 min-w-0">
                                    <p className="text-xs font-bold text-gray-900 group-hover:text-blue-700 transition-colors">
                                      <span className="font-black underline decoration-blue-200">{log.user?.name || 'System'}</span> {log.action.replace(/_/g, ' ').toLowerCase()}d {log.entity.toLowerCase()}
                                    </p>
                                    <p className="text-[10px] text-muted-foreground font-medium mt-0.5 line-clamp-1 italic">{log.details || 'No additional details'}</p>
                                    <div className="flex items-center gap-2 mt-2">
                                      <Badge variant="outline" className="text-[9px] h-4 bg-gray-50/50 text-gray-500 border-gray-100">
                                        {new Date(log.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                                      </Badge>
                                      <span className="text-[9px] text-gray-300">|</span>
                                      <span className="text-[9px] font-bold text-gray-400 uppercase tracking-tighter">{log.entity}</span>
                                    </div>
                                  </div>
                                </div>
                              </div>
                            ...
                            <div className="flex flex-col items-center justify-center py-12 text-muted-foreground">
                              <Shield className="size-8 opacity-20 mb-2" />
                      ...
                      <div className="p-3 bg-gray-50/50 border-t flex justify-center">
                        <Button 
                          variant="ghost" 
                          size="sm" 
                          className="h-7 text-[10px] font-bold uppercase tracking-widest text-blue-600 hover:text-blue-700 hover:bg-blue-100/50 w-full"
