...
import { WifiOff, Wifi, RefreshCw, X, Upload, Clock } from 'lucide-react';
...
export function OfflineBanner() {
  ...
  return (
    <div className={`sticky top-0 z-50 w-full transition-all duration-300 ${
      isSyncing || manualSyncing ? 'bg-amber-50 border-b border-amber-200 dark:bg-amber-950/50 dark:border-amber-800' :
        ...
        <div className="flex items-center gap-2 min-w-0">
          {(isSyncing || manualSyncing) ? (
            ...
            <>
              ...
              <span className="text-emerald-800 dark:text-emerald-200 truncate">
                Back online — data is up to date
              </span>
            </>
          ) : (
            <>
              ...
              <span className="text-red-800 dark:text-red-200 truncate">
                You are offline
                {pendingMutationCount > 0 ? ` — ${pendingMutationCount} change${pendingMutationCount !== 1 ? 's' : ''} queued` : ' — changes will sync when connected'}
              </span>
            </>
          )}
          {lastSyncedAt && (
            <span className="hidden sm:inline-flex items-center gap-1 text-xs text-gray-500 dark:text-gray-400 ml-1">
        ...
        <div className="flex items-center gap-1">
          {isOnline && pendingMutationCount > 0 && (
            <button
              ...
              className="px-2 py-1 rounded-md text-xs font-medium bg-emerald-600 text-white hover:bg-emerald-700 disabled:opacity-50 transition-colors shrink-0"
            >
          ...
          <button
            ...
            className="ml-1 p-1 rounded-md hover:bg-black/5 dark:hover:bg-white/10 transition-colors shrink-0"
            aria-label="Dismiss"
          >
