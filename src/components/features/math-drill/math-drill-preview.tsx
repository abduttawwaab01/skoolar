...
export function MathDrillPreview() {
  ...
  const handlePNG = useCallback(async () => {
    await captureFromHTML(exportMathDrillAsPNG, config.sheetTitle.replace(/\s+/g, '-').toLowerCase());
  }, [captureFromHTML, config.sheetTitle]);

  const handlePDF = useCallback(async () => {
    await captureFromHTML(exportMathDrillAsPDF, config.sheetTitle.replace(/\s+/g, '-').toLowerCase(), config.paperSize);
