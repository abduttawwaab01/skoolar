...
export function SpellingPreview() {
  ...
  const htmlContent = useMemo(() => renderSpellingHTML(config), [config]);
  ...
  const handlePNG = useCallback(async () => {
    await captureFromHTML(exportSpellingAsPNG, config.sheetTitle.replace(/\s+/g, '-').toLowerCase());
  }, [captureFromHTML, config.sheetTitle]);

  const handlePDF = useCallback(async () => {
    await captureFromHTML(exportSpellingAsPDF, config.sheetTitle.replace(/\s+/g, '-').toLowerCase(), config.paperSize);
  }, [captureFromHTML, config.sheetTitle, config.paperSize]);

  const handlePrint = useCallback(() => {
    printSpelling(renderSpellingHTML(config));
  }, [config]);
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="px-3 py-1.5 border-t text-[10px] text-muted-foreground bg-muted/10 flex justify-between">
        ...
        <span>{wordCount} words · {config.numberOfColumns} cols · {config.orientation} · {config.paperSize.toUpperCase()}</span>
      </div>
    </div>
  );
}
