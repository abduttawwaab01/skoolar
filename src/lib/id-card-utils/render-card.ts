...
export async function renderIDCard(
  ...
  const SYSTEM_FONTS = "'Segoe UI', 'Helvetica Neue', Arial, 'Liberation Sans', Roboto, sans-serif";
  const FF = fontCSS.length>0 ? `'SkoolarCard', ${SYSTEM_FONTS}` : SYSTEM_FONTS;
  
  console.log(`[IDCard] Using font-family: ${FF}`);
...
function buildPortraitModern(o:any):string {
  ...
  const hH = Math.round(H * 0.11);
  const mg = Math.round(W * 0.055);
  ...
  if(isBack){
    ...
    const sHtml=sections.map((sec:any)=>{
      const t=`<text x="${n(mg)}" y="${n(secY)}" font-size="${n(H*0.017)}" font-weight="700" fill="${prim}" letter-spacing="3">${sec.title}</text>
    ...
    return `<svg width="${W}" height="${H}" viewBox="0 0 ${W} ${H}" xmlns="http://www.w3.org/2000/svg">
      ...
      <text x="${n(W/2)}" y="${n(hH*.58)}" font-size="${n(H*.024)}" font-weight="700" fill="${hdrTxt}" text-anchor="middle">${schN}</text>
      <text x="${n(W/2)}" y="${n(hH*.88)}" font-size="${n(H*.013)}" fill="${hdrTxt}" text-anchor="middle" opacity="0.8" letter-spacing="3">IDENTIFICATION CARD — REVERSE</text>
      ${sHtml}
      <text x="${n(W/2)}" y="${n(H*.88)}" font-size="${n(H*.014)}" fill="${muted}" text-anchor="middle" opacity="0.7">If found, please return to the school office.</text>
      <rect x="0" y="${n(H-28)}" width="${W}" height="28" fill="${prim}" opacity="0.08"/>
      <text x="${n(W/2)}" y="${n(H-11)}" font-size="${n(H*.010)}" fill="${muted}" text-anchor="middle" opacity="0.6">Skoolar • School Management Platform</text>
    </svg>`;
  }
  ...
  if(showQR && qrB64){
    ...
    <text x="${n(W/2)}" y="${n(scanY)}" font-size="${n(H*.020)}" font-weight="700" fill="${prim}" text-anchor="middle" letter-spacing="4">SCAN TO VERIFY</text>`;
  }
  ...
  return `<svg width="${W}" height="${H}" viewBox="0 0 ${W} ${H}" xmlns="http://www.w3.org/2000/svg">
    ...
    <path d="M0 ${n(hH)} Q${n(W*.20)} ${n(hH+10)} ${n(W*.50)} ${n(hH+3)} Q${n(W*.80)} ${n(hH-4)} ${W} ${n(hH)}" fill="${prim}" opacity="0.4"/>
    
    <text x="${n(mg)}" y="${n(hH*.56)}" font-size="${n(H*.025)}" font-weight="700" fill="${hdrTxt}">${schN}</text>
    <text x="${n(W-mg)}" y="${n(hH*.56)}" font-size="${n(H*.016)}" font-weight="600" fill="${hdrTxt}" text-anchor="end" opacity="0.9" letter-spacing="2">ID CARD</text>
    ...
    <text x="${n(W/2)}" y="${n(H-11)}" font-size="${n(H*.010)}" fill="${muted}" text-anchor="middle" opacity="0.55">Skoolar • School Management Platform</text>
  </svg>`;
}
...
function buildLandscapeModern(o:any):string {
  ...
  const mg = Math.round(H * 0.06);

  if(isBack){
    const bLines=(backText||'').split('\n').filter((l:string)=>l.trim());
    let secY=Math.round(H*0.26); 
    const lh=Math.round(H*0.078);
    const sections=[
      {title:'CONTACT',lines:[schA,sPh,sEm].filter(Boolean)},
      {title:'IMPORTANT',lines:bLines}
    ...
    const sHtml=sections.map((sec:any)=>{
      const t=`<text x="${n(mg)}" y="${n(secY)}" font-size="${n(H*0.042)}" font-weight="700" fill="${prim}" letter-spacing="3">${sec.title}</text>
    ...
    return `<svg width="${W}" height="${H}" viewBox="0 0 ${W} ${H}" xmlns="http://www.w3.org/2000/svg">
      ...
      <rect width="${W}" height="${H}" fill="${sec}"/>
      <rect x="0" y="0" width="${W}" height="${n(hH)}" fill="url(#hg)"/>
      <text x="${n(W/2)}" y="${n(hH*.58)}" font-size="${n(H*.058)}" font-weight="700" fill="${hdrTxt}" text-anchor="middle">${schN}</text>
      <text x="${n(W/2)}" y="${n(hH*.88)}" font-size="${n(H*.032)}" fill="${hdrTxt}" text-anchor="middle" opacity="0.8" letter-spacing="3">IDENTIFICATION CARD — REVERSE</text>
      ${sHtml}
      <text x="${n(W/2)}" y="${n(H*.90)}" font-size="${n(H*.032)}" fill="${muted}" text-anchor="middle" opacity="0.7">If found, please return to the school office.</text>
      <rect x="0" y="${n(H-36)}" width="${W}" height="36" fill="${prim}" opacity="0.08"/>
      <text x="${n(W/2)}" y="${n(H-13)}" font-size="${n(H*.024)}" fill="${muted}" text-anchor="middle" opacity="0.55">Skoolar • School Management Platform</text>
    </svg>`;
  }
  ...
  return `<svg width="${W}" height="${H}" viewBox="0 0 ${W} ${H}" xmlns="http://www.w3.org/2000/svg">
    ...
    <path d="M0 ${n(hH)} Q${n(W*.20)} ${n(hH+10)} ${n(W*.50)} ${n(hH+4)} Q${n(W*.80)} ${n(hH-5)} ${W} ${n(hH)}" fill="${prim}" opacity="0.4"/>
    
    <text x="${n(mg)}" y="${n(hH*.56)}" font-size="${n(H*.058)}" font-weight="700" fill="${hdrTxt}">${schN}</text>
    <text x="${n(W-mg)}" y="${n(hH*.56)}" font-size="${n(H*.036)}" font-weight="600" fill="${hdrTxt}" text-anchor="end" opacity="0.9" letter-spacing="2">ID CARD</text>
    ...
    <text x="${n(txtX + badgeW/2)}" y="${n(badgeY + badgeH*0.66)}" font-size="${n(H*.034)}" font-weight="700" fill="${prim}" text-anchor="middle" letter-spacing="1">${pRole}</text>
    
    ${infoCard(infoCardX, infoCardY, infoCardW, infoCardH, sec, border, infoRowsHtml)}
    
    ${qrEl}
    
    <rect x="0" y="${n(H-36)}" width="${W}" height="36" fill="${prim}" opacity="0.06"/>
    <text x="${n(W/2)}" y="${n(H-13)}" font-size="${n(H*.024)}" fill="${muted}" text-anchor="middle" opacity="0.55">Skoolar • School Management Platform</text>
  </svg>`;
}
