export async function exportAttendanceAsPNG(element: HTMLElement, filename = 'attendance-register'): Promise<void> {
  try {
...
export async function exportAttendanceAsPDF(element: HTMLElement, filename = 'attendance-register', paperSize: 'a4' | 'letter' = 'a4'): Promise<void> {
  try {
    const { toPng } = await import('html-to-image');
...
export function printAttendance(html: string): void {
