...
const AUTHORIZED_ROLES = ['SUPER_ADMIN', 'SCHOOL_ADMIN', 'TEACHER', 'DIRECTOR'];
...
export async function POST(request: NextRequest) {
  try {
    const authResponse = await requireRole(request, AUTHORIZED_ROLES);
...
export async function GET(request: NextRequest) {
  try {
    const authResponse = await requireRole(request, [...AUTHORIZED_ROLES, 'STUDENT', 'PARENT']);
    if (authResponse instanceof NextResponse) return authResponse;
