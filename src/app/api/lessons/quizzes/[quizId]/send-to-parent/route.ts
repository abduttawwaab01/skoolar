...
export async function POST(
  ...
  try {
    ...
    if (auth.role !== 'SUPER_ADMIN' && auth.schoolId && quiz.lesson.schoolId !== auth.schoolId) {
      return NextResponse.json({ error: 'Forbidden' }, { status: 403 });
    }

    const baseUrl = process.env.NEXT_PUBLIC_APP_URL || request.headers.get('origin') || 'http://localhost:3000';
    const schoolName = quiz.lesson.school?.name || 'School';
    let totalSent = 0;
    let totalFailed = 0;
    ...
    for (const attempt of quiz.attempts) {
      ...
      const viewUrl = `${baseUrl}/dashboard?view=lesson-quiz&quizId=${quizId}`;
      ...
      const subject = `Lesson Quiz - ${quiz.title} - ${studentName}`;
...
The lesson quiz result for <strong>${studentName}</strong> — <strong>${quiz.title}</strong> is available.</p>
...
<p style="font-size:14px;color:#666;margin:4px 0 0">Passed: <strong>${passed}</strong></p></div>
...
<p style="font-size:12px;color:#aaa;text-align:center">Sent via Skoolar on behalf of ${schoolName}.</p>
      ...
      const whatsappMsg = `Dear Parent, ${studentName}'s lesson quiz result for "${quiz.title}" is ready. Score: ${score}, Passed: ${passed}. View: ${viewUrl}`;
