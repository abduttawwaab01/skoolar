...
export async function GET(
  ...
  try {
    ...
    // If no ID provided, find the teacher linked to the current auth userId
    const teacher = teacherId 
      ? await db.teacher.findUnique({ where: { id: teacherId } })
      : await db.teacher.findFirst({ where: { userId: auth.userId, schoolId: auth.schoolId } });
    ...
    const [classTeacherClasses, subjectTeacherClasses] = await Promise.all([
      db.class.findMany({
        where: { classTeacherId: resolvedTeacherId, schoolId: teacher.schoolId },
        select: { id: true, name: true, _count: { select: { students: true } } },
      }),
      db.classSubject.findMany({
        where: { teacherId: resolvedTeacherId, class: { schoolId: teacher.schoolId } },
    ...
    const exams = await db.exam.findMany({
      where: { teacherId: resolvedTeacherId, schoolId: teacher.schoolId },
