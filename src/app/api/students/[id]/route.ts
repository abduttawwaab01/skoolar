...
const AUTHORIZED_ROLES = ['SUPER_ADMIN', 'SCHOOL_ADMIN', 'TEACHER', 'DIRECTOR', 'STUDENT', 'PARENT'];
...
export async function GET(
  ...
  try {
    ...
    const isStaff = AUTHORIZED_ROLES.slice(0, 4).includes(auth.role || '');
    const isOwnRecord = auth.userId === student.userId;
...
export async function PUT(
  ...
  try {
    ...
    if (!isStaff) {
      return NextResponse.json({ error: 'Only staff can update student records' }, { status: 403 });
    }
...
export async function DELETE(
  ...
  try {
    ...
    if (!isStaff) {
      return NextResponse.json({ error: 'Only administrators can delete student records' }, { status: 403 });
    }
